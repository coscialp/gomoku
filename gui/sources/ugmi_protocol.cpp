...
const QByteArray UgmiProtocol::Send::position(std::string board, std::vector<std::string> moves)
{
    if (board == "startpos")
...
const QByteArray UgmiProtocol::Send::go(std::vector<std::string> searchmoves, std::map<std::string, int> options)
{
    ...
    {
        // TODO: Check if the option is valid and logger a warning if option are not valid
