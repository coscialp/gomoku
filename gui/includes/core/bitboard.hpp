...
class BitBoard {
 ...
 public:
    ...
    constexpr BitBoard() {}
    explicit constexpr BitBoard(std::array<uint64_t, 6> arr) : _board(arr) {}
    ...
    inline std::array<uint64_t, 6> raw_buffer() const { return this->_board; }
    inline bool get_square(Square sq) const {
        return this->_board[sq.value / 64] >> (sq.value % 64) & 1;
    }
    inline void set_square(Square sq) {
        this->_board[sq.value / 64] |= uint64_t(1) << (sq.value % 64);
    }
    inline void rst_square(Square sq) {
        this->_board[sq.value / 64] &= ~(uint64_t(1) << (sq.value % 64));
    }
    inline void swp_square(Square sq) {
        this->_board[sq.value / 64] ^= uint64_t(1) << (sq.value % 64);
    }

    friend std::ostream &operator<<(std::ostream &os, const BitBoard &bb) {
        for (Rank r = SquareConstants::FIRST_RANK;
             r <= SquareConstants::LAST_RANK; r++) {
            for (File f = SquareConstants::FIRST_FILE;
                 f <= SquareConstants::LAST_FILE; f++) {
