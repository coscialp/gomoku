...
struct Rank
{
    ...
    constexpr Rank(uint8_t _value) : value(_value) {}
    friend std::ostream &operator<<(std::ostream &os, const Rank &r)
    {
        return os << ('0' + (r.value + 1) / 10) << ('0' + (r.value + 1) % 10);
    }
    constexpr Rank operator+(uint8_t value) { return Rank(this->value + value); }
    constexpr Rank operator-(uint8_t value) { return Rank(this->value - value); }
    ...
    constexpr bool operator==(const Rank &rhs) const { return value == rhs.value; }
    constexpr bool operator!=(const Rank &rhs) const { return value != rhs.value; }
    constexpr bool operator<(const Rank &rhs) const { return value < rhs.value; }
    constexpr bool operator>(const Rank &rhs) const { return value > rhs.value; }
    constexpr bool operator<=(const Rank &rhs) const { return value <= rhs.value; }
